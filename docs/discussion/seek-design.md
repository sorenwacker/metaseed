...
Date: 260720. Status: historical design draft.

!!! note "Superseded"
    This is an early design exploration. The shipped SEEK adapter took a
    different route — the native **FAIR Data Station RDF import** — documented in
    [SEEK Export](../architecture/seek-export.md), which also covers the
    provisioning, import and update flows. Kept here for design context; the
    `SeekConfigModel`/resolver/`apply` structure sketched below was not built.
