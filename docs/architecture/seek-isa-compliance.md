...
An `Input` field may declare the entity it references (`reference: ObservationUnit`); the Sample Type built for that entity then links to the referenced entity's type rather than to the level before it. Assay-level entities under one profile Assay are grouped into the linear chains SEEK can hold — an entity referencing a study-level entity starts a chain, one referencing an assay-level entity continues it — and every chain beyond the first in a Study — a second chain under the same profile Assay, or another profile Assay of that Study (LC-MS and NMR of the same samples, say) — becomes its own assay stream: SEEK's `fix_assay_linkage_for_new_assays` splices an assay whose input type another assay in the stream already takes *in front of* that assay, so a stream is a line and never a branch. This is how a combined "assay with data file" (input: the study sample) coexists with a material → data file chain.
...
### Column for column: deriving the entity from the template file

The JSON:API never exposes an installed template's attributes, so a profile that must match the installed templates exactly is derived from the **template files** that configured the instance (the same format *Download ISA Templates* writes). `metaseed seek-import-templates profile.yaml templates/*.json --write` re-derives every entity whose `seek.template` names one of the files: each attribute becomes a field with the template's `required`, `description`, `isaTag` and title flag; a SEEK type the field type cannot name (`ENA custom date`, `Text`, `Registered Data file`, `Registered Sample List`) is kept in `seek_attribute_type`; a `Controlled Vocabulary` attribute keeps its `CVList` as the field's `enum` *and* binds, through `seek_controlled_vocab`, the vocabulary SEEK created when the template was populated (named after the attribute), with `seek_cv_free_text` for `allowCVFreeText`. What only the profile knows — an `Input` field's `reference`, ontology terms, identity markers — is carried over; a field the template lacks is dropped, because the column does not exist in SEEK.

The sync then creates the Sample Type with exactly those columns: the recorded attribute type, the instance vocabulary resolved by title (an absent one is an error naming the column, and provisioning makes no copy of it), and each attribute's `template_attribute_id` read from `POST /templates/{id}/template_attributes` — the way SEEK's own from-template form records it. A `Registered Data file` column holds a reference to a SEEK DataFile record, so the sync registers the field's URL as a remote data file (SEEK fetches the URL's headers when registering, so it must be reachable from the instance) and stores the record's id; the same applies to such an attribute in Extended Metadata. Other record references (`Registered Sample`, `Registered Strain`) are reported and left empty.

The gate is the round trip: the derived entity rendered back through `to_isa_template_json` reproduces the template's attributes (`tests/test_seek/test_definitions.py`).
