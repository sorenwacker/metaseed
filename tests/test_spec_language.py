...
class TestValidationRuleUniqueness:
    """Tests for uniqueness validation rule."""
    ...
    def test_unique_values_pass(self) -> None:
        ...
        errors = rule.validate({"identifier": "ID-002"})
        assert len(errors) == 0
...
class TestValidationRuleReference:
    ...
    def test_valid_reference(self) -> None:
        ...
        rule = EntityReferenceRule(
            ...
            available_ids={"LOC-001", "LOC-002"},
        )
        data = {"location": {"unique_id": "LOC-001", "name": "Field A"}}
        errors = rule.validate(data)
        assert len(errors) == 0
    ...
    def test_invalid_reference(self) -> None:
        ...
        data = {"location": {"unique_id": "LOC-999", "name": "Unknown"}}
        errors = rule.validate(data)
        assert len(errors) == 1
        assert "LOC-999" in errors[0].message

    def test_missing_reference_skipped(self) -> None:
        """Missing reference is skipped."""
        from metaseed.validators.rules import EntityReferenceRule
