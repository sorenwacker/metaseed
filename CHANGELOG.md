...
### Fixed
- **A run date reaches the ENA `Run` as a date, not as ENA's raw text.** The Portal reports every column as a string and the mapper coerced the other six non-string fields (`taxon_id`, `insert_size`, `insert_size_stddev`, both `lat_lon` components, `environmental_sample`) but not `run_date`, which the profile types as a date — so a string was written into a date field, surviving the mapper and `validate()` and only failing at export, where the SRA schema types the element. Both forms ENA writes are read (`YYYY-MM-DD` and `2010-02-01T00:00:00`, whose time is dropped), and an unreadable value is left unset rather than written in the wrong type. The shipped `arabidopsis-drought-rnaseq` example quoted its six run dates, so it carried the same defect. A test derives the typed fields from the profile, so a new one cannot be added without a coercer.
...
- A study table links its Source to its Sample again: a `Protocol REF` column sits between them naming the study's `sample collection` protocol, which ISA-Tab requires to connect the two materials. The cell is left empty when the study declares no protocol of that type, rather than referencing one of the wrong type.
- The `Term Source REF` column of a study table was always written blank, even beside an accession, leaving a reader with an identifier it could not resolve. The ontology is now read back from the accession (`PATO:0000461`, or an OBO PURL), and `ONTOLOGY SOURCE REFERENCE` declares the sources the export actually uses instead of being emitted empty.
...
- **A PRIDE `submission.px` is no longer malformed by a multi-line value.** The file is line-based with no continuation syntax, but nothing collapsed newlines inside a value, so a two-line description — an ordinary abstract — emitted its second line as a bare record the px-submission-tool cannot read. Whitespace inside a value is now collapsed to single spaces, which also stops a tab from inventing a column.
- A PRIDE dataset's publication reached no output: its PubMed ID and DOI were held on the Dataset and never written, so a published submission lost its own citation. Both are emitted as `MTD` lines.
- The PRIDE SDRF table carries every column SDRF-Proteomics marks REQUIRED, in the order the specification lays them out. Five were missing (`comment[proteomics data acquisition method]`, `comment[label]`, `comment[cleavage agent details]`, `comment[fraction identifier]`, `comment[technical replicate]`) along with `characteristics[biological replicate]`. Where the profile models no field the cell reads `not available`, which the specification names as the value for a mandatory column with unknown content — dropping the column instead fails validation. Fraction and replicate columns default to `1`, the specification's value for one unfractionated, unreplicated measurement.
