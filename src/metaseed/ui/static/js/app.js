...
function toggleGraph() {
    ...
    if (container.classList.contains('hidden')) {
        ...
        loadGraph();
        startGraphPolling(); // Auto-refresh for MCP changes
    } else {
...
function validateDatasetName() {
    ...
    // Validate format: alphanumeric start, then alphanumeric/hyphens/underscores
...
function loadDataset(name) {
    fetch(BASE_URL + '/api/datasets/load', {
        ...
        .then(function(data) {
            ...
            // Reload page to refresh the entity tree
            window.location.reload();
        })
        .catch(function(err) {
            showNotification(err.message, 'error');
